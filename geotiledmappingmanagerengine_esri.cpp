    ...
    QGeoTiledMappingManagerEngine()
{
    ...
    cameraCaps.setMaximumTilt(0); // avoid map tilting
    ...
    /* MAP TILE HOST URL */
    if (parameters.contains(QStringLiteral("esri.mapping.custom.host"))) {
        m_customHostUrlScheme = buildCustomHostUrlScheme( parameters.value( QStringLiteral( "esri.mapping.custom.host" ) ).toString() );
    } else {
        m_customHostUrlScheme = "";
    }
...
QString
GeoTiledMappingManagerEngineEsri::buildCustomHostUrlScheme(const QString &customHostAdress)
{
    static const QString tileScheme = "tile/%1/%3/%2";
    if (customHostAdress.endsWith("/"))
        return QString("%1%2").arg(customHostAdress).arg(tileScheme);
    else
        return QString("%1/%2").arg(customHostAdress).arg(tileScheme);
}
...
QString GeoTiledMappingManagerEngineEsri::customHostAddress() const
{
    return m_customHostUrlScheme;
}
