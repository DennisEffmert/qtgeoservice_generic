...
QGeoTiledMapReply *GeoTileFetcherEsri::getTileImage(const QGeoTileSpec &spec)
{
    ...
    const QString customHostAddress = engine->customHostAddress();
    if ( customHostAddress.isEmpty() )
    {
        ...
        request.setUrl(customHostAddress.arg(spec.zoom()).arg(spec.x()).arg(spec.y()));
    }
